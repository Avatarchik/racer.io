...
public class CarMovementController : MonoBehaviour
{
    ...
    public void Evade(CarScript other)
    {
        _totalInput += DoEvade(other);
    }

    Vector2 DoEvade(CarScript other)
    {
    ...
    public void Pursuit(CarScript other)
    {
        _totalInput += DoPursuit(other);
    }

    Vector2 DoPursuit(CarScript other)
    {
        float distance = Vector2.Distance(other.MovementController.Rigidbody.position, Rigidbody.position);
    ...
    void TestDash()
    {
        StartCoroutine(DashRoutine(0.3f, CarSettings.Instance.MaxSpeed * 5));
    }
    ...
    public void Dash(float duration, float speed)
    {
        ...
        _dashRoutine = DashRoutine(duration, speed);
        StartCoroutine(_dashRoutine);
    }

    IEnumerator DashRoutine(float duration, float speed)
    {
        float timer = duration;

        Vector2 curDir = _velocity.normalized;

        _clampToMaxSpeed = false;

        while (timer > 0)
        {
            ResetTotalInput();

            SetVelocity(curDir * speed); 
