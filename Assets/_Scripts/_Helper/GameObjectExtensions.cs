...
public static class GameObjectExtensions
{
    ...
    /// <returns></returns>
    public static T[] GetInterfacesMM<T>(this GameObject gObj)
    {
    ...
    public static T GetInterface<T>(this GameObject gObj)
    {
        if (!typeof(T).IsInterface)
            throw new SystemException("Specified type is not an interface!");
        return gObj.GetInterfacesMM<T>().FirstOrDefault();
    }
    ...
    public static void SetActiveImmediateChildren(this GameObject gObj, bool ac)
    {
        foreach (Transform tr in gObj.transform)
        {
            tr.gameObject.SetActive(ac);
        }
    }

    public static void SetAllActiveRecursively(this GameObject gObj, bool ac)
    {
        gObj.SetActive(ac);

        foreach (Transform childTransform in gObj.transform)
        {
            SetAllActiveRecursively(childTransform.gameObject, ac);
        }
    }
}